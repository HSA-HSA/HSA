...
tf.flags.DEFINE_string('attack_method', 'HSA', 'The name of attack method.')
tf.flags.DEFINE_string('layer_name','InceptionV3_mix/InceptionV3_mix/Mixed_5b/concat','The layer to be attacked.')
tf.flags.DEFINE_string('layer_name_ori','InceptionV3/InceptionV3/Mixed_5b/concat','The layer to be attacked.')

tf.flags.DEFINE_string('input_dir', './dataset/images/', 'Input directory with images.')

tf.flags.DEFINE_string('output_dir', './adv/HSA_PD/', 'Output directory with images.')
...
"""parameter for NAA"""
tf.flags.DEFINE_float('ens', 40.0, 'Number of aggregated n.')
...
def name_in_checkpoint(var):
  if "InceptionV3_mix" in var.op.name:
    return var.op.name.replace("InceptionV3_mix", "InceptionV3")
...
def main(_):
    ...
    layer_name_ori=FLAGS.layer_name_ori
    ...
    with tf.Graph().as_default():
        ...
        index_in = tf.placeholder(dtype=tf.int64, shape=[FLAGS.batch_size*2])
        scale_in = tf.placeholder(dtype=tf.float32, shape=[FLAGS.batch_size*2])
        
        network_fn_ori = utils.nets_factory.get_network_fn_ori(FLAGS.model_name, num_classes=num_classes, is_training=False)
        network_fn = utils.nets_factory.get_network_fn(FLAGS.model_name+"_mix", num_classes=num_classes, is_training=False)
        
        x=tf.concat([ori_input,adv_input],axis=0)

        # whether using DIM or not
        if 'DI' in FLAGS.attack_method:
            logits, end_points = network_fn(input_diversity(x), index_in, scale_in)
            logits_ori, end_points_ori = network_fn_ori(input_diversity(x))
        else:
            logits, end_points = network_fn(x, index_in, scale_in)
            logits_ori, end_points_ori = network_fn_ori(x)
        ...
        opt_operations_ori,_ = get_opt_layers(layer_name_ori)
        ...
        loss = get_NAA_loss(opt_operations_ori,weights_ph,base_feature)
        ...
        variables = slim.get_variables(scope="InceptionV3")
        variables_to_resotre = [v for v in variables if 'InceptionV3_mix' not in v.name]
        saver=tf.train.Saver(variables_to_resotre)
        variables_mix = slim.get_variables(scope="InceptionV3_mix")
        variables_mix = {name_in_checkpoint(var):var for var in variables_mix}
        #print(variables_mix)
        saver_mix = tf.train.Saver(variables_mix)
        with tf.Session() as sess:
            saver.restore(sess,checkpoint_path)
            saver_mix.restore(sess,checkpoint_path)
            count=0
                ...
                for i in range(num_iter):
                    if i==0:
                        for l in range(int(FLAGS.ens)):
                            ...
                            images_tmp2 += np.random.normal(size = images.shape, loc=0.0, scale=0.3)
                            scale_factor = 0.5 + (0.5/(FLAGS.ens//FLAGS.batch_size+1))*(l//FLAGS.batch_size+1)
                            index1 = [(j+l)%FLAGS.batch_size for j in range(FLAGS.batch_size)]
                            index2 = [(j+l)%FLAGS.batch_size+FLAGS.batch_size  for j in range(FLAGS.batch_size)]
                            index = index1+index2
                            index = np.array(index, dtype=np.int64)
                            scale_factors = [scale_factor for i in range(FLAGS.batch_size*2)]
                            scale_factors = np.array(scale_factors, dtype=np.float32)
                            w, feature = sess.run([weights_tensor, opt_operations[0]],feed_dict={ori_input: images_tmp2, adv_input: images_tmp2, label_ph: labels, index_in: index, scale_in: scale_factors})
                    ...
                    index1 = [j for j in range(FLAGS.batch_size)]
                    index2 = [j+FLAGS.batch_size  for j in range(FLAGS.batch_size)]
                    index = index1+index2
                    index = np.array(index, dtype=np.int64)
                    scale_factors = [1.0 for i in range(FLAGS.batch_size*2)]
                    scale_factors = np.array(scale_factors, dtype=np.float32)
                    feature_base = sess.run([opt_operations[0]],
                                            feed_dict={ori_input: images_base, adv_input: images_base,label_ph: labels, index_in: index, scale_in: scale_factors})
